...
- `src/web/partials/*.html` hold the major shell fragments and composition roots: header, rail, sidebar, details panel, settings modal, fork modal, shared head/style blocks, plus deeper splits like `styles.html -> styles_*` and `primary_surface.html -> composer/workflow-lab` partials, with the runtime/session strip now living in header chrome.
