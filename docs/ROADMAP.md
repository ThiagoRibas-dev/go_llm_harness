...
* **Specialized ingestion reference:** [Crawl4AI](https://github.com/unclecode/crawl4ai) — not a peer harness, but an excellent reference for **LLM-ready web ingestion**: JS-rendered crawling, clean Markdown output, deep crawl, structured extraction, screenshots, media/link capture, caching, and crawl-time controls. Its README is also a good warning label: once you expose browser-backed crawling or a Docker API surface, SSRF/file-write/XSS/auth bugs become very real product work, not theory.
...
* **GoHarness plan:** two tools first — `web_search.go` and `web_fetch.go` — with an explicit later path for deeper crawling.
  ...
  - **Phase B (default fetch engine):** `net/http` with manual redirect following + SSRF guard (block RFC1918/loopback/link-local, validate DNS per hop, cap 5 MB, 30 s timeout, max 3 concurrent) → `github.com/nickstenning/html-to-markdown` (or a Go Readability port) → Markdown. Detect GitHub URLs and `git clone --depth 1` into a per-session cache. Detect PDFs and use `github.com/ledongthuc/pdf` for local text.
  - **Phase C (JS-heavy fallback):** route through Jina Reader (`https://r.jina.ai/<url>`) or another explicitly opt-in remote extractor when the page is JS-rendered and the user/model actually needs rendered content. Keep this behind a mode flag so the default product remains local-first and pure-Go.
  - **Phase D (later, optional):** a deeper crawl mode influenced by Crawl4AI: bounded BFS/priority traversal, page caps, host restrictions, screenshots, media/link capture, and structured extraction. This should be a separate tool or explicit mode (`web_crawl`) rather than silently turning single-page fetch into a browser crawl.
...
* **Effort:** M for Phase A/B, L once Phase C/D (JS-rendered fallback or deep crawl) enters scope.
...
* **Reference:** `pi-subagents` FleetView + `pi-background-tasks`; agents run after the parent turn settles with a persistent inspector for transcripts/steer/stop. `prime-agent` reinforces the same direction with **daemon-backed sessions**, **reattachable workers**, retained child agents, direct agent-to-agent delivery modes (`auto` / `steer` / `follow_up`), and durable session artifacts instead of treating the terminal client as the owner of the work.
* **GoHarness plan:** on top of the shipped engine, add a job registry (in-memory map + `.goharness/jobs/<id>.json`): `id, parent_session, spec, status, started_at, finished_at, result_path`. `spawn_sub_agent` gains `background: bool`; background jobs return immediately and continue in a goroutine. Reuse existing `subagent_start`/`subagent_done` SSE events; add `subagent_log`. A web "Fleet" panel lists jobs with stop/steer. Steering queue (11.2) feeds into this. Prefer explicit **mailboxes / receipts / artifact exchange** over magical shared mutable context; if children share state, make it an explicit scratch artifact or host-owned message channel.
* **Effort:** M.
...
* **Reference:** dsh `goal/` family — `ctx.goals` holds durable objective state as part of the session log; a `goal-round-driver` continues the agent across turns until the goal is met or blocked; `tool-goal`/`command-goal` expose it to model and user. `prime-agent` independently validates this lane with **persistent goals**, **heartbeats**, **schedules**, and **bounded autonomous mode** where host-owned budgets and quality gates decide whether work continues. This is "give the agent an objective and let it work across multiple rounds until done" — beyond our single ReAct loop.
* **Plan:** add a goal record (description, done criteria, status) to session state; a driver loop that, after each turn settles, asks the model "is the goal met? if not, continue" until completion or a step budget; surface progress in the UI. Requires the steering/queue (11.2) and cancellation (11.3) to be safe. Add optional recurring re-entry later (`heartbeat` / scheduled follow-up) and make quality gates host-owned, never inferred from a model claim that the task is done.
* **Effort:** M.
...
* **Reference:** `codex exec` runs without the TUI, prints progress to `stderr`, final output to `stdout`, supports `--json` JSONL events, `--ephemeral`, `--output-last-message`, and `--output-schema <json-schema>`. `prime-agent` adds another strong data point with `--mode json` / print-mode event streams over JSONL for integrations and custom UIs.
* **Why it matters:** this is the cleanest automation surface Codex has. It is much stronger than "just shell out to the interactive client" because CI can consume structured machine events instead of screen text. Prime Agent reinforces that this should be a **first-class event stream**, not an afterthought.
...
* **Plan:** add `goharness exec "prompt"` with: `--json` (event stream), `--ephemeral` (no persisted session files), `-o/--output-last-message`, and `--output-schema schema.json` enforcing a final JSON object. Reuse the `Agent` runtime, the spill store, and the future typed event log. Emit progress on `stderr`, final answer on `stdout`, exactly like Codex, and keep the event stream stable enough that a third-party UI can consume it without screen-scraping.
* **Effort:** M.

### 13.2 App-server / RPC seam for rich local and remote clients **[NEW]**
* **Reference:** `codex app-server` exposes a JSON-RPC 2.0-like protocol over **stdio JSONL**, **WebSocket**, or **Unix socket**, supports generated TypeScript/JSON schemas, and lets the TUI connect remotely via `codex --remote ...`. `prime-agent` adds the architecture lesson of a **daemon supervisor + worker runtime + client-side connection boundary**, where the client renders and the resident worker owns queues, schedules, descendants, and recovery.
* **Why it matters:** Codex cleanly separates **agent runtime** from **client surface**. The CLI, IDE extension, desktop app, and remote clients can all talk to the same core protocol instead of scraping terminal output. Prime Agent reinforces the same separation with daemon-backed reattachable sessions and a local protocol between client and supervisor.
* **GoHarness today:** embedded web server + SSE + OpenAI-compatible HTTP gateway, but no first-class agent-control RPC protocol.
* **Plan:** after the typed session/event log lands, add `goharness app-server` with a narrow typed protocol: `thread/start`, `turn/send`, `turn/cancel`, `session/list`, `session/resume`, `tool/approve`, `spill/read`, `review/start`. Start with **stdio JSONL** and **Unix socket**; add WebSocket only when we have a concrete external client. Unlike Codex, keep this provider-neutral and local-first. Longer-term, split client concerns from resident worker concerns more explicitly: the client owns rendering and local preferences; the runtime owns queues, schedules, background jobs, and descendant agent recovery.
* **Effort:** L.
