# 🧭 GoHarness Execution Roadmap

This document is the **execution-grade roadmap** for GoHarness.

- Use **this file** to decide what to implement next.
- Keep `docs/ROADMAP.md` as the **historical + comparative research ledger**.

The old roadmap was very good at capturing **product ideas, comparative insights, and UX references**. It was weaker at capturing **implementation readiness, storage models, event schemas, migration plans, and dependency chains**. This document reorganizes the backlog around those realities.
...
- **Pi / DSH / Codex / Claude** — competitive analysis already folded into `docs/ROADMAP.md`
- **Crawl4AI / tldw / Prime** — adjacent references already folded into `docs/ROADMAP.md`
...
|---|---|---|---|---|---|
| 9.1 | O(1) range loader + hierarchical memory decay | **Spec needed** | Historical roadmap, OptMem references in `docs/RESEARCH.md` | 11.6, 12.2 | Need storage model, hierarchy geometry, invalidation rules, prompt reconstruction budgets |
| 9.2 | Visual memory map dashboard | **Spec needed** | Historical roadmap, newer UI direction in 12.28/12.29 | 9.1, 12.2, shell placement | Need backing API, UI placement, semantics for forget/regenerate/branch-from-summary |
| 11.6 | Session tree / time-travel navigator | **Spec needed** | Historical roadmap, Pi references, DSH event-log pressure | 12.2 strongly related | Need canonical event schema, branch semantics, dual-write migration plan |
| 11.9 | Cross-session memory | **Ready** | Historical roadmap, Pi memory refs, Code/BM25 | — | Good v1 path: BM25-backed dated memory notes; embedding/rerank can remain optional |
| 12.2 | Session event log as source of truth | **Spec needed** | DSH, Codex/Prime event-stream references, Historical roadmap | 11.6 strongly related | Need exact event schema, projections, migration from per-turn JSON files |
| 12.18 | Session query with FTS | **Blocked** | DSH, Historical roadmap, current BM25 engine | 11.6, 12.2 | Straightforward after canonical event log exists |
| 12.29.7 | Full step-grouped transcript / compaction placement / streaming-tail isolation | **Spec needed** | DSH, Historical roadmap | 12.2 | Partial transcript cleanup is possible now, but the full model needs a typed event stream |
| 12.28.12 | Deliverables, references, message feedback | **Ready** | DSH, tldw, Historical roadmap | 11.4 helps | Deliverables row is implementable now; exportable artifact bundles come later |
...
| ID | Item | Status | Source basis | Depends on | Missing decisions / notes |
|---|---|---|---|---|---|
| 11.1 | Web search & fetch | **Ready** | Pi, Crawl4AI, tldw, Historical roadmap | Spill foundation already landed | One of the best-specified items; default pure-Go fetch, optional JS-heavy fallback later |
| 11.4 | `@file` mentions in composer | **Ready** | Pi, Code, Historical roadmap | — | UI typeahead + backend file query |
| 11.16 | Parent-directory context walking + overrides | **Ready** | Pi, current `LoadLocalInstructions()`, Historical roadmap | — | Small, clear extension |
| 11.19 | Image paste/drag into composer | **Ready** | Pi, current uploads plumbing, Historical roadmap | 12.19 improves storage | Ready even before content-addressing, though 12.19 gives the cleaner backend |
| 12.5 | LSP seam | **Ready** | DSH, Historical roadmap | — | Clear four-op seam; just medium effort |
| 12.19 | Content-addressed attachments | **Ready** | DSH, current uploads path, Historical roadmap | — | Good bounded v1 |
| 14.9 | MCP prompts, resources, remote transports, OAuth, elicitation | **Blocked / staged** | Claude MCP docs, Historical roadmap | 11.11 for elicitation; 13.8 for transport breadth | Prompt/resource surfacing is ready sooner than full OAuth+elicitation stack |
| 13.8 | MCP breadth: streamable HTTP, OAuth, richer management | **Ready** | Codex + Claude MCP docs, Historical roadmap | — | Staged implementation strongly recommended |
...
| ID | Item | Status | Source basis | Depends on | Missing decisions / notes |
|---|---|---|---|---|---|
| 10.2B | Wait First / Race | **Ready** | Historical roadmap, current subagent runtime, Prime references | — | Implement through current subagent engine; define cancellation and winner semantics clearly |
| 10.2C | Async / Fire-and-Forget | **Subsumed** | Historical roadmap, Prime, 11.7 | 11.7 | Do not build separately; fold into durable background jobs |
| 11.2 | Message queue while agent is running | **Ready** | Pi, Code, Historical roadmap | — | Good v1 spec already exists |
| 11.3 | Cancel vs abort distinction | **Ready** | Pi, Code, Historical roadmap | — | Small and clear |
| 11.5 | `!command` shell injection | **Ready** | Pi, Code, Historical roadmap | — | Small, careful UX needed |
| 11.7 | Durable/background sub-agent jobs | **Ready** | Pi, Prime, current subagent runtime, Historical roadmap | 11.2 helpful | Prefer explicit mailboxes / receipts / artifact exchange, not magical shared mutable context |
...
| 11.10 | Automatic build/lint/typecheck feedback loop | **Ready** | Pi, Code, Historical roadmap | — | Heuristic per-language v1 is good enough |
...
| 11.12 | Persistent TODO/plan overlay | **Ready** | Pi, DSH composer references, Historical roadmap | — | Straightforward first pass |
| 12.4 | First-class persistent terminals (PTY) | **Ready** | DSH, tldw, Historical roadmap | 12.1 helpful but not required | Important distinction between model-invoked PTY tools and any explicitly user-owned persistent terminal surface |
| 12.7 | Plan mode as logged collaboration state | **Blocked** | DSH, Codex, Claude, Historical roadmap | 12.3, 11.11, UI takeover plumbing | Behavior is clear; interception and UI surfaces should exist first |
| 12.8 | Goals with autonomous round-driving | **Blocked** | DSH, Prime, Codex, Historical roadmap | 11.2, 11.7 | Enough info exists, but it wants durable continuation machinery |
| 12.9 | Dynamic workflows over sub-agents | **Blocked** | DSH, current subagents, Historical roadmap | 11.7, 12.10, 12.11 | Better after background jobs and structured output land |
...
| 12.14 | Scope primitive | **Blocked** | DSH, Historical roadmap | 12.3, 12.13 | Small internal spec still needed once scoped registrations exist |
| 12.20 | Code Mode / model-written programs | **Spec needed** | DSH, Historical roadmap | 12.1 | Need runtime choice, security model, SDK/binding surface |
| 12.21 | Scheduled / mission runs | **Blocked** | DSH, Prime, Historical roadmap | 11.7, 12.8 | Good once jobs exist |
| 13.5 | Dedicated code-review mode | **Ready** | Codex, Claude, Historical roadmap | — | Strongly spec'd and high value |
...
| 14.5 | Subagents as data files | **Ready** | Claude docs, current runtime, Historical roadmap | — | Strong improvement path over ad hoc role config |
| 14.6 | Worktree-aware isolation | **Spec needed** | Claude docs, Historical roadmap | Review/branch/job semantics | Need interaction with current rollback model, worktree cleanup and collision policy |
| 14.7 | Review + security-review as first-class flows | **Ready** | Claude + Codex, Historical roadmap | — | Very implementable |
| 14.8 | Background agents, task roster, queued command semantics | **Blocked** | Claude, Pi, Prime, Historical roadmap | 11.2, 11.7 | Clear destination; durable task machinery first |
...
| ID | Item | Status | Source basis | Depends on | Missing decisions / notes |
|---|---|---|---|---|---|
| 11.13A | Prompt templates | **Ready** | Pi, Codex, Claude, Historical roadmap | — | Tiny first slice |
| 11.13B | Skills (description + lazy load) | **Ready** | Pi, Codex, Claude, Historical roadmap | — | Enough for a bounded first pass |
...
| 11.14 | Provider/model catalog as data | **Ready** | Historical roadmap, current provider/profile code | — | Medium, straightforward |
| 11.15 | Project trust & local config policy | **Ready** | Pi, Claude trust docs, Historical roadmap | — | Enough for a basic trust gate |
| 12.1 | Capability seams (FS / Runner / Terminals / LSP) | **Blocked** | DSH, Code, Historical roadmap | Refactor bandwidth | Concept and interfaces are clear; implement incrementally |
| 12.3 | Waterfall event hooks | **Blocked** | DSH, Codex, Claude, Historical roadmap | 12.2 ideal, but narrow v1 can precede it | Enough for a v1 bus; full fidelity wants better logging semantics |
| 12.15 | Profiles & bundles as layered patchable config | **Spec needed** | DSH, Historical roadmap | 14.1 helps | Need merge/patch semantics, precedence rules, file format |
...
| 12.22 | Guard / secret scanning | **Ready** | DSH, current guardrails, Historical roadmap | — | Easy high-value guard item |
| 12.23 | Human feedback out of model context | **Ready** | DSH, Historical roadmap | — | Clean sidecar feature |
| 12.24 | Skills as on-demand capability packages | **Subsumed** | DSH, Pi, Historical roadmap | 11.13, 13.4 | Do not spec separately; implement through the unified skills/plugin track |
| 12.25 | Telemetry / observability contracts | **Ready** | DSH, current trace logging, Historical roadmap | — | Good medium-scope cleanup |
...
| 13.2 | App-server / RPC seam | **Blocked** | Codex, Prime, Historical roadmap | 12.2 recommended | A narrow v1 is possible sooner; clean long-term version wants a better event/log substrate |
...
| 14.1 | Repo-scoped configuration hierarchy | **Ready** | Claude docs, Historical roadmap | — | Good structural cleanup item |
...
| 14.4 | Skills / commands / dynamic context injection | **Ready** | Claude, Codex, Historical roadmap | 11.13B helpful | Very good v1 spec |
...
| ID | Item | Status | Source basis | Depends on | Missing decisions / notes |
|---|---|---|---|---|---|
| 11.17 | Richer live status & cost footer / status chips | **Ready** | Pi, tldw, Code, Historical roadmap | — | Strong first pass exists; later can graduate from footer to clickable chips |
...
| 11.20 | Editor modal & slash command palette | **Ready** | Pi, Frontend plan, Historical roadmap | — | Good shell-level enhancement |
...
These are the places where the old roadmap still needs real engineering specs before implementation starts.
...
1. **Do not start spec-needed items straight from the old prose.** Write a small design spec first.
